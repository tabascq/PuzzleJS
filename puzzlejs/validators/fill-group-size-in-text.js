puzzleValidators["fill-group-size-in-text"] = {
    ...
    validate: function(puzzleGrid, param) {
        ...
        puzzleGrid.getFillGroups().forEach(group => {
            ...
            else if (!group.complete) { group.cells.forEach(cell => { cell.fail(); }); }    // group is incomplete
            else if (group.cells.length < targetCount) { groupResult = true; }              // group too small for a single target - bad if complete
